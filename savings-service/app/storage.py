...
def cleanup_goal_images_except_current(goal_id: str, current_image_url: str | None) -> None:
    ensure_bucket_exists()

    current_object_name = get_object_name_from_url(current_image_url)
    ...
    try:
        ...
        for item in objects:
            ...
            print(f"[MinIO] Cleaned old goal image: {item.object_name}")

    except S3Error as error:
        print(f"[MinIO] Failed to clean goal images for {goal_id}: {error}")
